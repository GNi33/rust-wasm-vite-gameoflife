...
impl Universe {
    pub fn tick(&mut self) {
        ...
        let mut next = {
            let _timer = Timer::new("Universe::tick - clone store");
            self.store.clone()
        };

        {
            let _timer = Timer::new("Universe::tick - generate new generation");
        ...
        let _timer = Timer::new("Universe::tick - update store");
        self.store = next;
    }
